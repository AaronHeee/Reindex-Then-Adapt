        ...
        for data_name in args.data_names:
            for split in ["train", "valid", "test"]:
                ...
                if not os.path.exists(path):
                    print(f"Skip {data_name} {split} as {path} does not exist.")
                    continue
                for line in tqdm(
...
def main(
    ...
    label: str = "labels_from_llm_as_single_token",
    freeze_llm_embed: bool = True,
    aggregator: str = "weighted",
    dropout_prob: float = 0.1,
    data_names: list = ["inspired", "redial", "redditv1.5", "wikipedia"],
    epochs: int = 200,
    batch_size: int = 256,
    ...
    trainer = pl.Trainer(
        ...
        logger=CSVLogger(
            args.ckpt_dir,
            name=os.path.join(
                "_".join(args.data_names).replace("/", "_"), args.aggregator
            ),
        ),
    )
